...
class SCAttention_v1(nn.Module):
    """Predator + loftr Self-Cross Attention Implementation"""
    ...
    def __init__(
        ...
        for atten_type in layer_names:
            if atten_type == "self":
                ...
                # layers.append(AttentionalPropagation(feature_dim, num_head))
                layers.append(LoFTREncoderLayer(feature_dim, num_head, attention='linear'))
        self.layers = nn.ModuleList(layers)
    ...
    def forward(
        ...
        for layer, name in zip(self.layers, self.names):
            if name == "cross":
                feat0 = desc0.permute(0, 2, 1)
                feat1 = desc1.permute(0, 2, 1)
                feat0 = layer(feat0, feat1)
                feat1 = layer(feat1, feat0)
                desc0 = feat0.permute(0, 2, 1)
                desc1 = feat1.permute(0, 2, 1)
            elif name == "self":
                desc0 = layer(coords0, desc0)
                desc1 = layer(coords1, desc1)
            else:
